...
1. Bump `version` in `package.json` (this repo has no `CHANGELOG.md` yet — if one is
   added later, keep it in sync with what's actually live on npm). This can go directly
   to `main` as usual — branch protection on this repo only requires a reviewed PR for
   changes under `.github/` (see below), not for every change.
